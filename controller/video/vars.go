...
import "github.com/liuyang/ly/public"
...
type videoShareUrlParser interface {
	parseShareUrl(shareUrl string) (*public.VideoParseInfo, error)
}

// videoIdParser 根据视频ID解析
type videoIdParser interface {
	parseVideoID(videoId string) (*public.VideoParseInfo, error)
}

// BatchParseItem 批量解析时, 单条解析格式
type BatchParseItem struct {
	ParseInfo *public.VideoParseInfo // 视频解析信息
